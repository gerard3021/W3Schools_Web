...
<form action="<?= base_url('categories/update/' . $item['CategoryID']) ?>" method="post">
    ...
    <div class="campo"><label>Descripción: *</label><textarea name="Description" required><?= esc($item['Description']) ?></textarea></div>
    <br><input type="submit" value="Actualizar">&nbsp;<a href="<?= base_url('categories') ?>" class="btn btn-gray">Cancelar</a>
</form>
