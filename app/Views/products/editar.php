...
<form action="<?= base_url('products/update/' . $item['ProductID']) ?>" method="post">
    ...
    <div class="campo"><label>Unidad:</label><input type="text" name="Unit" value="<?= esc($item['Unit']) ?>"></div>
    <div class="campo"><label>Precio:*</label><input type="number" name="Price" step="0.01" min="0" value="<?= esc($item['Price']) ?>" required></div>
